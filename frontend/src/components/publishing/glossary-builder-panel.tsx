...
interface GlossaryBuilderPanelProps {
...
export function GlossaryBuilderPanel({
  entries,
  mode,
  onModeChange,
  onEntriesChange,
  ...
  onSave,
  isSaving = false,
}: GlossaryBuilderPanelProps) {
  const [termInput, setTermInput] = useState('');
  const [definitionInput, setDefinitionInput] = useState('');

  const handleAddEntry = () => {
    if (!termInput.trim() || !definitionInput.trim()) {
      toast.error('Enter both term and definition');
      return;
    }
  ...
  const handleClearAll = () => {
    if (window.confirm('Clear all glossary entries?')) {
      onEntriesChange([]);
  ...
  return (
    <div className="bg-surface-container-lowest border border-outline-variant/10 rounded-xl p-8 mb-8">
      <div className="flex items-center justify-between mb-6 gap-3">
        <div>
          <h3 className="font-label text-lg font-bold text-primary uppercase tracking-widest">Glossary Builder</h3>
          <p className="text-xs text-on-surface-variant mt-1">
            {mode === 'auto' 
              ? 'Auto-extracted glossary from document terms' 
              : 'Manually manage glossary entries for export'}
          </p>
        </div>
        {!isEditing ? (
          <button
                  ...
                  <div key={`${entry.term}-${index}`} className="rounded-lg bg-surface-container-low p-3 border border-outline-variant/20">
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-on-surface-variant mt-1">{entry.definition}</p>
                      </div>
                      <button
